...
export default function PlaywithK() {
    const {t, common, locale} = usePageBasics("playwithk");
...
function HeroSection({t}) {
    return <Hero title="PlaywithK" subtitle={t("hero.subtitle")}/>;
}
        ...
        <section
            ...
            <div className="flex flex-col items-center md:items-start md:w-1/3">
                ...
                <div className="flex gap-6 mt-2 sm:mt-4">
                    {socials.map(({href, label, iconSrc}) => (
                        <a
                            key={label}
                            href={href}
                            target="_blank"
...
function TimelineSection({t}) {
    ...
    return (
        <section
            ...
            <div
                className="absolute top-0 left-1/2 sm:left-1/2 left-4 -translate-x-1/2 sm:-translate-x-1/2 translate-x-0 w-1 bg-teal-600 h-full"></div>
